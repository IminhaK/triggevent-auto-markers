...
public class O12SP2 extends AutoChildEventHandler implements FilteredEventHandler {
    ...
    public void abilityCast(EventContext context, AbilityCastStart event) {
        ...
        switch (id) {
            case 13164 -> call = targetAnalysis;
            case 13143 -> call = ionEfflux;
            case 13144 -> call = archivePeripheral;
            case 13210 -> call = indexAndArchivePeripheral;
            case 13156 -> call = oversampledWaveCannonTank;
            default -> {
                return;
            }
        }
    ...
    @AutoFeed
    private final SequentialTrigger<BaseEvent> patchSq = SqtTemplates.multiInvocation(35_000, AbilityCastStart.class,
    ...
    private void patch1(AbilityCastStart e1, SequentialTriggerController<BaseEvent> s) {
    ...
    private void patch3(AbilityCastStart e1, SequentialTriggerController<BaseEvent> s) {
    ...
    private void helloWorld1(AbilityCastStart e1, SequentialTriggerController<BaseEvent> s) {
        ...
        if(playerJob.isTank()) {
            ...
            if(getBuffs().statusesOnTarget(getState().getPlayer()).stream().anyMatch(ba -> ba.buffIdMatches(latentData))) {
                ...
                hw1DPS1(e1, s);
            } else if(getBuffs().statusesOnTarget(getState().getPlayer()).stream().anyMatch(ba -> ba.buffIdMatches(CSB) && ba.getInitialDuration().toSeconds() < 12)) {
                hw1DPS2(e1, s);
            } else {
                hw1DPS4(e1, s);
            }
    ...
    private final ModifiableCallout<AbilityCastStart> hw1dps4third = ModifiableCallout.durationBasedCall("HW1: Nothing DPS 2", "Go to C");
